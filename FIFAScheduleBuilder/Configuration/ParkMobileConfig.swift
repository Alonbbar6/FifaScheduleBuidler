...
/// Configuration manager for ParkMobile API credentials
/// Loads credentials from environment variables or uses defaults from .env file
struct ParkMobileConfig {
    static let shared = ParkMobileConfig()
    ...
    var isValid: Bool {
        return !apiKey.isEmpty && apiKey != "YOUR_PARKMOBILE_API_KEY_HERE"
    }

    /// Returns a user-friendly status message about the API configuration
    var statusMessage: String {
        if isValid {
            return "ParkMobile API configured successfully ✓"
        } else {
            return "ParkMobile API key not configured. Please set ParkMobile environment variable."
        }
    }
