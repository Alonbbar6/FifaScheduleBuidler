...
class IndoorWayfindingService {
    ...
    private func loadStadiumDataAsync() async {
        ...
        if let hardRockData = await loadStadiumJSONAsync(filename: "hard_rock_stadium_sample") {
            ...
            stadiumDataCache["stadium-001"] = hardRockData // Also map to our existing ID
            print("✅ IndoorWayfinding: Loaded Hard Rock Stadium data")
            print("   - Cached with IDs: hard-rock-stadium, stadium-001")
        } else {
    ...
    private func loadStadiumJSONAsync(filename: String) async -> StadiumNavigationData? {
        ...
        // Try to load from home/ubuntu folder first
        let ubuntuPath = "/Users/alonsobardales/Desktop/_clean_tracker/home/ubuntu/fifa_2026_stadium_data/sample_data/\(filename).json"
        let ubuntuURL = URL(fileURLWithPath: ubuntuPath)

        print("🔍 IndoorWayfinding: Checking path: \(ubuntuPath)")

        // Try loading from ubuntu path
        if let data = try? Data(contentsOf: ubuntuURL) {
            print("✅ IndoorWayfinding: File loaded successfully, size: \(data.count) bytes")
            do {
                let decoder = JSONDecoder()
                let stadiumData = try decoder.decode(StadiumNavigationData.self, from: data)
                print("✅ IndoorWayfinding: JSON decoded successfully")
                print("   - Stadium ID: \(stadiumData.stadiumId)")
                print("   - Stadium Name: \(stadiumData.stadiumName)")
                print("   - Gates: \(stadiumData.gates.count)")
                print("   - Sections: \(stadiumData.sections.count)")
                print("   - Amenities: \(stadiumData.amenities.count)")
                return stadiumData
            } catch {
                print("❌ IndoorWayfinding: Failed to decode \(filename): \(error)")
            ...
            print("⚠️ IndoorWayfinding: Could not load file from ubuntu path")
        }
